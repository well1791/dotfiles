...
| laya-router (pi ext) | laya service, pi | Pi model-routing extension (chezmoi `home/dot_pi/agent/extensions/laya-router/`): consults laya per prompt, routes laya/small/frontier; config `~/.pi/agent/laya.json`; falls back to normal pi model on any laya failure; benchmark `docs/laya-pi-benchmark-2026-09.md` |
